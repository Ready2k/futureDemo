      ...
      <AnimatePresence>
        {expanded && (
          ...
          >
            ...
            {futureMode && trace.modelRouting && (
              <Section title="Intelligence Layer" icon={Cpu} iconColor="#7c3aed">
                <Row label="Intent model" value={trace.modelRouting.intent} color="#a78bfa" />
                <Row label="Reasoning" value={trace.modelRouting.reasoning} color="#a78bfa" />
                <div style={{ height: '0.5rem' }} />
                <div style={{ fontSize: '0.72rem', color: '#555', marginBottom: '5px', textTransform: 'uppercase', letterSpacing: '0.08em', fontWeight: '600' }}>Data Sources</div>
                {trace.modelRouting.sources.map((s, i) => (
                  <div key={i} style={{ fontSize: '0.78rem', color: '#888', padding: '3px 0', display: 'flex', gap: '7px', alignItems: 'center' }}>
                    <div style={{ width: '5px', height: '5px', borderRadius: '50%', background: '#7c3aed', flexShrink: 0 }} />
                    {s}
                  </div>
                ))}
                <div style={{ height: '0.5rem' }} />
                <div style={{ fontSize: '0.72rem', color: '#555', marginBottom: '5px', textTransform: 'uppercase', letterSpacing: '0.08em', fontWeight: '600' }}>Tools Called</div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                  {trace.modelRouting.tools.map((t, i) => (
                    <span key={i} style={{ fontSize: '0.72rem', background: 'rgba(124,58,237,0.12)', color: '#9c6fef', padding: '2px 8px', borderRadius: '100px', fontFamily: 'monospace' }}>{t}</span>
                  ))}
                </div>
              </Section>
            )}
            <div style={{ marginTop: '0.5rem', padding: '8px', background: 'rgba(255,255,255,0.03)', borderRadius: '6px', fontSize: '0.72rem', color: '#555', textAlign: 'center' }}>
              {futureMode ? 'Multi-agent AI stack · Barclays Intelligence Platform v3.0' : 'Trace generated by AI Governance Layer v1.0'}
            </div>
          </motion.div>
        )}
      </AnimatePresence>
